...
import { AlertTriangle, Boxes, CheckCircle2, Edit3, Flame, Loader2, PackagePlus, Plus, RefreshCw, Search, Trash2 } from 'lucide-react';
          ...
          <form onSubmit={handleSubmit} className="mt-5 space-y-4">
            ...
            <Field label="Company / brand">
              <select value={form.company} onChange={(e) => update('company', e.target.value)} className="input">
                {LPG_COMPANIES.map((x) => <option key={x}>{x}</option>)}
              </select>
            </Field>
            <div className="grid grid-cols-2 gap-3">
