...
export default function LpgInventory() {
  ...
  const printStockReport = () => {
    ...
    if (!w) return;
    w.document.write(`<!doctype html><html><head><title>LPG Cylinder Stock Report</title><style>
      @page{size:A4;margin:12mm}body{font-family:Arial,sans-serif;color:#0f172a;margin:0}.header{text-align:center;margin-bottom:16px}.shop{font-size:24px;font-weight:900}.muted{color:#64748b;font-size:12px}.summary{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:14px 0}.box{border:1px solid #cbd5e1;border-radius:12px;padding:10px}.box span{display:block;color:#64748b;font-size:11px;text-transform:uppercase;font-weight:800}.box b{font-size:20px}table{width:100%;border-collapse:collapse;font-size:11px}th{background:#111827;color:white;text-align:left;padding:8px}td{border-bottom:1px solid #e2e8f0;padding:7px;vertical-align:top}.right{text-align:right}.red{color:#dc2626;font-weight:900}.green{color:#047857;font-weight:900}.badge{display:inline-block;border-radius:999px;padding:3px 8px;font-size:10px;font-weight:900}.badge-red{background:#fee2e2;color:#b91c1c}.badge-green{background:#dcfce7;color:#166534}.footer{margin-top:14px;text-align:center;color:#64748b;font-size:11px}@media print{button{display:none}}
    </style></head><body>
      <div class="header"><div class="shop">${escapeHtml(business.business_name || 'LPG Cylinder Shop')}</div><div class="muted">${escapeHtml(business.address || '')}</div><div class="muted">${escapeHtml(business.phone || '')}</div><h2>LPG Cylinder Inventory Stock Report</h2><div class="muted">Printed at ${new Date().toLocaleString()}</div></div>
      <div class="summary"><div class="box"><span>Total items</span><b>${rows.length}</b></div><div class="box"><span>Full cylinders</span><b>${totalFull}</b></div><div class="box"><span>Empty cylinders</span><b>${totalEmpty}</b></div><div class="box"><span>StockOut</span><b class="red">${stockOut}</b></div></div>
      <table><thead><tr><th>#</th><th>Company</th><th>Size</th><th>Type</th><th>SKU</th><th class="right">Full stock</th><th class="right">Empty cylinders</th><th class="right">Price</th><th>Status</th></tr></thead><tbody>
        ${rows.map((x, i) => `<tr><td>${i + 1}</td><td><b>${escapeHtml(x.company)}</b></td><td>${escapeHtml(x.cylinder_size)}</td><td>${escapeHtml(x.item_type)}</td><td>${escapeHtml(x.sku || '-')}</td><td class="right ${Number(x.full_stock || 0) <= 0 ? 'red' : 'green'}">${Number(x.full_stock || 0)}</td><td class="right">${Number(x.empty_stock || 0)}</td><td class="right">${formatMoney(Number(x.price || 0), currency)}</td><td>${Number(x.full_stock || 0) <= 0 ? '<span class="badge badge-red">StockOut</span>' : '<span class="badge badge-green">In Stock</span>'}</td></tr>`).join('')}
      </tbody></table><div class="footer">Generated by CounterPOS LPG Cylinder Inventory</div><script>window.onload=function(){setTimeout(function(){window.print()},250)}</script></body></html>`);
    w.document.close();
  };
